...
export const createProviderStore = ({ now = Date.now, ttl = PROVIDER_TTL_MS, maxKeys = MAX_KEYS, maxPerKey = MAX_PER_KEY } = {}) => {
  /** @typedef {Map<string, { ts: number, addedAt: number }>} ProviderBucket */
  /** @type {Map<string, ProviderBucket>} */
  const byKey = new Map(); // key -> Map<providerDid, { ts, addedAt }>

  /** @param {ProviderBucket} bucket */
  const live = (bucket) => {
    const t = now();
  ...
  return {
     ...
     * @param {unknown} entry — a wire-decoded provider claim (verified below)
     */
    async add(entry) {
      // why narrow after the await: an async type predicate doesn't flow
      // through `await`, but verifyProvider has now confirmed the shape.
      if (!(await verifyProvider(entry))) return { ok: false, reason: 'bad-signature' };
      const claim = /** @type {import('./records.js').ProviderEntry} */ (entry);
      let bucket = byKey.get(claim.key);
      if (!bucket) {
        if (byKey.size >= maxKeys) return { ok: false, reason: 'full' };
        bucket = new Map();
        byKey.set(claim.key, bucket);
      }
      live(bucket);
      if (!bucket.has(claim.provider) && bucket.size >= maxPerKey) {
        // Evict the oldest to make room — a busy key keeps its freshest providers.
        /** @type {[string, { ts: number, addedAt: number }] | null} */
        let oldest = null;
        for (const [did, v] of bucket) if (!oldest || v.addedAt < oldest[1].addedAt) oldest = [did, v];
        if (oldest) bucket.delete(oldest[0]);
      }
      bucket.set(claim.provider, { ts: claim.ts, addedAt: now() });
      return { ok: true };
    },
