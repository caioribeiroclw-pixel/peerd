...
const makeDhtDialer = (room) => async (contact) => {
  ...
  catch (e) { dlog('dht', `relay-dial of ${contact.did.slice(-8)} via ${broker.slice(-8)} failed: ${e?.message ?? e}`); return false; }
...
export const createDwebClient = () => {
  ...
  return Object.freeze({
    ...
    BASE_TOPIC,
    joinBaseNetwork: async ({ identity, url = DEFAULT_SIGNALING[0], audit = null } = {}) => {
